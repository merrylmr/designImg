	...
	<div class="nav" :style="{background:(pcMode==false?color.bgColor:'#262b30')}">
		...
		<div class="login" v-if="logState=='logged' || logState=='c_logged' || logState=='c_visitor'">
			<div class="center">
				...
				<span class="item" v-if="(logState!='c_logged' && logState!='c_visitor')" @click.stop="callModal({'type':'share','modalOver':false})" :style="{color:color.fontColor}">
					<i class="tbzico ico-eShare" style="margin-top: 1px;"></i>分享</span>
				<span class="item" v-if="((isCoop && coopDownload )|| !isCoop) && (logState!='c_logged' && logState!='c_visitor')" @click.stop="onDownloadClick" :style="{color:color.fontColor}">
				...
				<span class="item" v-if="logState=='logged' || logState=='c_logged' || logState=='c_visitor'" @click.stop="callModal({'type':'savebox','modalOver':false})" :style="{color:color.fontColor}">
			...
			<div class="right">
				<span class="item" v-if="(logState!='c_logged' && logState!='c_visitor')" :style="{color:color.fontColor}" @click="$store.commit('callModal',{type:'colorset',modalOver:false})"><i class="tbzico ico-skin-in"></i></span>
				<span v-if="logState=='logged' || logState=='c_logged'" class="avatar item" :style="{color:color.fontColor}" @click="$store.commit('callModal',{type:'userset',modalOver:false})">
				...
				<span class="item-text black" v-if="logState=='c_visitor' || logState=='c_logged'" @click="onFinishedBtnClickHandler">
